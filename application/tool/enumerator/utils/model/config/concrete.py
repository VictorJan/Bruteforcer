...
from application.utils.protocol.descriptor import PrivateNonDataDescriptor
from application.utils.model.requirement import NestedRequirement
...
class Config(metaclass=AbstractMetaConfig):
    ...
    def __init__(self,**data):
        ...
        instance with a respective PrivateNonDataDescriptor, given that class contains according slots and instance has no __dict__:
        ...
        if hasattr(self.__class__,'__slots__') and not hasattr(self,'__dict__'):
            for key,value in data.items():
                ...
                setattr(self.__class__, key, PrivateNonDataDescriptor(key))
    ...
    def to_dict(self):
        ...
        :return:
        '''
        attrs=filter(lambda item: not item[0].startswith('_') and isinstance(item[1], PrivateNonDataDescriptor), self.__class__.__dict__.items())
